%-----------------------------------------------------------------------------
%
%	rights.tex Document preface part
%
%	INCLUDE FILE FOR LaTeX2e DOCUMENT
%
%	AUTHOR: Ari Potkonen /JARVENPAA/ Mon Jun 28 2022
%-----------------------------------------------------------------------------
%        1         2         3         4         5         6         7
%23456789012345678901234567890123456789012345678901234567890123456789012345678
%-BEGIN OF INCLUDE FILE-------------------------------------------------------
\label{rights}
%\index{rights}
\addcontentsline{toc}{chapter}{Rights}
\vspace*{\fill}
Online: \url{http://github.com/apotkonen/gt} \\ %\cite{GT}\\
{\scriptsize\url{
 https://raw.githubusercontent.com/apotkonen/gt/main/general.pdf}
}\vspace*{\fill}

Editor: Not Named Yet\\
Author: Potkonen Ari\\
\copyright 2023 Potkonen Ari. All rights reserved.
\vspace{\baselineskip}

LOGO's:\\
FAST LIFE OF
Electron Positron Dwell "EP-Dwell" % \textsuperscript{\texttrademark}
\includegraphics[height=32pt,width=32pt]{fig/ep-dwell.eps} ,\\
\\
General Theory % \textsuperscript{\texttrademark}
\includegraphics[height=32pt,width=32pt]{fig/gt-dwell.eps}
and "GT-doubleWell" % \textsuperscript{\texttrademark}
\includegraphics[height=32pt,width=32pt]{fig/gt.eps}  ; - ) .
\vspace*{\fill}

All referred trademarks, logos and brand names are the property of their
respective owners. This document is licensed under CC-BY-SA licensing scheme.
\vspace{\baselineskip}

Any of the trademarks, service marks, collective marks, design rights, or
similar rights that are mentioned, used, or cited are the property of their
respective owners. Their use here does not imply that you may use them for any
purpose other than for the same or a similar informational use as contemplated
by the original author of document licensed under the CC-BY-SA licensing
scheme. Author cannot grant any rights to use any otherwise protected
materials. Your use of any such or similar incorporeal property is
at your own risk.
%-END OF INCLUDE FILE---------------------------------------------------------
